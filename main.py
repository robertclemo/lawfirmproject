    ...
    try:
        ...
        if(rows):
            ...
            for row in rows:
                ...
                Role:............................{Style.RESET_ALL}{Fore.BLUE}{row[5]}{Fore.WHITE}
                ''')
            mycursor.close()
        else:
...
def insertStaffTable():
    ...
    for i in range(count):
        ...
        roles = str(input("Staff Role >> "))
        cursor.execute(
            f"INSERT INTO staff (fname, lname, email, specialty, roles) values ('{fname}', '{lname}', '{email}', '{specialty}', '{roles}')")
        firmdb.commit()
